// ─────────────────────────────────────────────────────────────────────────────
// Perfectly Flawed Families — Firebase Edition
//
...
import { CREAM, CHARCOAL, TEAL, LTGREY } from "./src/constants/colors";
import { HomeIcon, BookIcon, RootsIcon, UsersIcon, ProfileIcon } from "./src/components/icons";
...
function AppContent() {
  ...
  return (
    <div style={{ width:"100%", maxWidth:390, margin:"0 auto", height:"100dvh", background:CREAM, position:"relative", overflow:"hidden", display:"flex", flexDirection:"column", boxShadow:"0 0 60px rgba(45,43,40,0.25)", filter: darkMode ? "invert(1) hue-rotate(180deg)" : "none" }}>
      ...
      <div style={{ flex:1, overflow:"hidden", position:"relative" }}>
        ...
        {user && (
          <>
            ...
            {tab === 1 && (subscribed ? <DevotionScreen user={user} pendingTopic={pendingTopic} onTopicConsumed={() => setPendingTopic(null)} /> : <PaywallScreen user={user} featureName="Family Devotions" />)}
            {tab === 2 && (subscribed ? <AdvisorScreen  user={user} /> : <PaywallScreen user={user} featureName="Family Roots" />)}
      ...
      {/* PWA install banner — shown to all visitors on first load */}
      {!isInstalled && user !== undefined && (
        <InstallBanner installPrompt={installPrompt} triggerInstall={triggerInstall} isInstalled={isInstalled} />
      )}

      {/* Bottom nav — only when logged in */}
      {user && (
        <div style={{ height:80, background:"rgba(253,250,245,0.97)", backdropFilter:"blur(20px)", borderTop:"1px solid rgba(46,125,107,0.2)", display:"flex", alignItems:"center", justifyContent:"space-around", padding:"0 10px 8px", flexShrink:0, boxShadow:"0 -4px 20px rgba(45,43,40,0.06)" }}>
          {tabs.map(({ label, Icon }, i) => (
            <button key={i} onClick={() => setTab(i)} style={{ background:"none", border:"none", cursor:"pointer", display:"flex", flexDirection:"column", alignItems:"center", gap:4, padding:"8px 12px", position:"relative" }}>
              {tab === i && <div style={{ position:"absolute", top:-1, left:"50%", transform:"translateX(-50%)", width:24, height:2, background:TEAL, borderRadius:1 }}/>}
              <Icon a={tab === i} />
              <span style={{ fontFamily:"Georgia,serif", fontSize:10, fontWeight:600, letterSpacing:"0.05em", color: tab===i ? TEAL : LTGREY }}>{label}</span>
            </button>
          ))}
        </div>
      )}
