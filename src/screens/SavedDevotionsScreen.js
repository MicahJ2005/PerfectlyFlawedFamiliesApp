...
export function shareDevotion(devotion) {
  const subject = encodeURIComponent(`Family Devotion: ${devotion.title}`);
  ...
  return (
    <div style={{ height:"100%", overflowY:"auto", padding:"0 20px 100px" }}>
      ...
      <div style={{ display:"flex", alignItems:"center", gap:12, padding:"28px 0 20px" }}>
        ...
        <span style={{ display:"inline-block", background:"rgba(46,125,107,0.12)", color:TEAL, fontFamily:"Georgia,serif", fontSize:11, fontWeight:600, letterSpacing:"0.12em", textTransform:"uppercase", padding:"4px 12px", borderRadius:20 }}>
          {devotion.topic}
        </span>
      </div>

      <div style={{ background:"rgba(255,255,255,0.82)", borderRadius:18, padding:0, overflow:"hidden", border:"1px solid rgba(46,125,107,0.18)" }}>
        ...
        <div style={{ padding:"22px 20px" }}>
          ...
          <div style={{ background:"rgba(45,43,40,0.04)", borderRadius:12, padding:14 }}>
            <div style={{ display:"flex", alignItems:"center", gap:7, marginBottom:8 }}>
              <CrossIcon size={14}/>
              <p style={{ fontFamily:"Georgia,serif", fontSize:11, fontWeight:600, color:TEAL, letterSpacing:"0.12em", textTransform:"uppercase", margin:0 }}>Closing Prayer</p>
            </div>
  ...
  return (
    <div style={{ height:"100%", overflowY:"auto", padding:"0 20px 100px" }}>
      ...
      <p style={{ fontFamily:"Georgia,serif", fontSize:13, color:MIDGREY, margin:"0 0 20px" }}>
        {devotions.length > 0 ? `${devotions.length} saved devotion${devotions.length > 1 ? "s" : ""}` : "Your family library"}
      </p>
      ...
      {!loading && devotions.length === 0 && (
        <div style={{ textAlign:"center", paddingTop:60 }}>
          ...
          <p style={{ fontFamily:"Georgia,serif", fontSize:13, color:LTGREY, lineHeight:1.6 }}>Generate a family devotion and tap Save to build your library.</p>
        </div>
      )}
      ...
      {devotions.map(d => (
        <div
          ...
          style={{ background:"rgba(255,255,255,0.82)", borderRadius:16, padding:0, marginBottom:12, border:"1px solid rgba(46,125,107,0.18)", overflow:"hidden", cursor:"pointer", boxShadow:"0 2px 10px rgba(45,43,40,0.06)" }}
        >
          <div style={{ background:"linear-gradient(135deg,#2E7D6B,#3A9E88)", height:3 }} />
          <div style={{ padding:"16px 18px" }}>
            <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between" }}>
              <span style={{ display:"inline-block", background:"rgba(46,125,107,0.1)", color:TEAL, fontFamily:"Georgia,serif", fontSize:10, fontWeight:600, letterSpacing:"0.12em", textTransform:"uppercase", padding:"3px 10px", borderRadius:20, marginBottom:8 }}>
                {d.topic}
              </span>
              {confirmId === d.id ? (
            ...
            <p style={{ fontFamily:"Georgia,serif", fontSize:12, color:TEAL, margin:"0 0 6px" }}>{d.scripture?.reference}</p>
