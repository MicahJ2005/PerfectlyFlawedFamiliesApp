...
import { RootsIcon, BookIcon, AdvisorIcon } from "../components/icons";
import { GoldButton } from "../components/ui";

// ─── Replace these with your actual Stripe Price IDs from the Stripe Dashboard ───
const MONTHLY_PRICE_ID = "price_1TAKrrHsfaE8SAOVsydeoTON";  // e.g. $9.99/month
const ANNUAL_PRICE_ID  = "price_1TAZSVHsfaE8SAOVpbJMcIPW";   // e.g. $69.99/year
// ─────────────────────────────────────────────────────────────────────────────────

const MONTHLY_LABEL  = "$9.99 / month";
const ANNUAL_LABEL   = "$69.99 / year";
const ANNUAL_SAVINGS = "Save 40%";
  ...
  return (
    <div style={{ height:"100%", overflowY:"auto", padding:"0 20px 100px" }}>
      ...
      <div style={{ textAlign:"center", padding:"36px 0 24px" }}>
        <div style={{ width:64, height:64, borderRadius:"50%", background:"linear-gradient(135deg,#2E7D6B,#3A9E88)", display:"flex", alignItems:"center", justifyContent:"center", margin:"0 auto 16px" }}>
        ...
        <p style={{ fontFamily:"Georgia,serif", fontSize:13, color:MIDGREY, margin:0, lineHeight:1.6 }}>
          {featureName ? `${featureName} is a premium feature.` : "These features require a membership."} Join to unlock everything.
        </p>
      </div>

      {/* Feature list */}
      <div style={{ background:"rgba(46,125,107,0.07)", borderRadius:16, padding:"18px 20px", marginBottom:20 }}>
        {[
          { Icon:BookIcon,    label:"Unlimited Family Devotions", desc:"Devotions tailored for every season of family life" },
          { Icon:AdvisorIcon, label:"Family Roots Sessions",      desc:"Faith-grounded guidance for family challenges"      },
          { Icon:RootsIcon,   label:"Save & Revisit",             desc:"Save devotions and revisit your past roots sessions"  },
        ].map(({ Icon, label, desc }) => (
          <div key={label} style={{ display:"flex", alignItems:"center", gap:12, marginBottom:14 }}>
            <div style={{ width:36, height:36, borderRadius:10, background:"rgba(46,125,107,0.15)", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
              <Icon size={18} />
            </div>
            <div>
            ...
            <span style={{ marginLeft:"auto", color:TEAL, fontSize:16 }}>✓</span>
          </div>
        ))}
      </div>
      ...
      <div style={{ display:"flex", gap:10, marginBottom:20 }}>
        {[
          { id:"monthly", label:"Family Monthly", price:MONTHLY_LABEL, badge:null },
          { id:"annual",  label:"Family Annual",  price:ANNUAL_LABEL,  badge:ANNUAL_SAVINGS },
        ].map(({ id, label, price, badge }) => (
          <button key={id} onClick={() => setPlan(id)} style={{ flex:1, border:`2px solid ${plan===id ? TEAL : "rgba(46,125,107,0.25)"}`, borderRadius:16, padding:"14px 12px", cursor:"pointer", background: plan===id ? "rgba(46,125,107,0.08)" : "transparent", position:"relative", textAlign:"center" }}>
            {badge && (
              <span style={{ position:"absolute", top:-10, left:"50%", transform:"translateX(-50%)", background:TEAL, color:WHITE, fontFamily:"Georgia,serif", fontSize:10, fontWeight:700, padding:"2px 10px", borderRadius:10, whiteSpace:"nowrap" }}>{badge}</span>
            )}
            <p style={{ fontFamily:"Georgia,serif", fontSize:13, fontWeight:700, color: plan===id ? TEAL : CHARCOAL, margin:"0 0 4px" }}>{label}</p>
      ...
      <p style={{ fontFamily:"Georgia,serif", fontSize:11, color:LTGREY, textAlign:"center", marginTop:12, lineHeight:1.6 }}>
        Cancel anytime. Family Prayer Wall is always free.{"\n"}Secure payment via Stripe.
      </p>
        ...
        {showCode && (
          <div style={{ marginTop:12, display:"flex", gap:8 }}>
            <input
              ...
              style={{ flex:1, border:"1.5px solid rgba(46,125,107,0.4)", borderRadius:10, padding:"10px 14px", fontFamily:"Georgia,serif", fontSize:13, color:CHARCOAL, background:WHITE, outline:"none", letterSpacing:"0.1em" }}
            />
            <button onClick={redeemCode} disabled={codeLoading || !code.trim()} style={{ background:TEAL, border:"none", borderRadius:10, padding:"10px 18px", fontFamily:"Georgia,serif", fontSize:13, fontWeight:700, color:WHITE, cursor:"pointer", opacity: codeLoading || !code.trim() ? 0.6 : 1 }}>
              {codeLoading ? "…" : "Apply"}
            </button>
          </div>
        )}
