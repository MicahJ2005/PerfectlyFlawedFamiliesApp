...
export function PrayerScreen({ user }) {
  ...
  const [newPrayer,           setNewPrayer]           = useState({ text:"", group:"Praying Parents", isAnon:false });
  ...
  const submitPrayer = async () => {
    ...
    setNewPrayer({ text:"", group:"Praying Parents", isAnon:false });
    setShowForm(false);
    setSubmitted(true);
    ...
    <div style={{ height:"100%", display:"flex", flexDirection:"column", position:"relative" }}>
      <div style={{ padding:"32px 20px 0", flexShrink:0 }}>
        <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", marginBottom:12 }}>
          <div>
            <h1 style={{ fontFamily:"Georgia,serif", fontSize:26, fontWeight:700, color:CHARCOAL, margin:"0 0 4px" }}>Family Prayer Wall</h1>
            <p style={{ fontFamily:"Georgia,serif", fontSize:13, color:MIDGREY, margin:0 }}>
              {activePrivate ? `🔒 ${activePrivate.name}` : "Lift your family up together in prayer"}
            </p>
          </div>
          <button onClick={() => setShowForm(true)} style={{ background:TEAL, border:"none", borderRadius:"50%", width:44, height:44, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", boxShadow:"0 4px 12px rgba(46,125,107,0.4)", flexShrink:0 }}>
        ...
        <div style={{ display:"flex", gap:8, overflowX:"auto", paddingBottom:12, scrollbarWidth:"none" }}>
          {["All", ...visibleGroups].map(g => {
            ...
            return (
              <button key={g} onClick={() => selectPublicTab(g)} style={{ flexShrink:0, border:`1.5px solid ${active ? TEAL : "rgba(46,125,107,0.25)"}`, borderRadius:20, padding:"6px 14px", fontFamily:"Georgia,serif", fontSize:12, fontWeight:600, color: active ? WHITE : MIDGREY, background: active ? TEAL : "transparent", cursor:"pointer", whiteSpace:"nowrap" }}>
                {g}
              </button>
            );
          })}
          ...
          {privateGroups.length > 0 && (
            <div style={{ width:1, background:"rgba(46,125,107,0.25)", flexShrink:0, margin:"4px 4px" }} />
          )}

          {privateGroups.map(group => {
            const active = activePrivate?.id === group.id;
            return (
              <button key={group.id} onClick={() => selectPrivateTab(group)} style={{ flexShrink:0, border:`1.5px solid ${active ? TEAL : "rgba(46,125,107,0.25)"}`, borderRadius:20, padding:"6px 14px", fontFamily:"Georgia,serif", fontSize:12, fontWeight:600, color: active ? WHITE : MIDGREY, background: active ? TEAL : "rgba(46,125,107,0.06)", cursor:"pointer", whiteSpace:"nowrap", display:"flex", alignItems:"center", gap:5 }}>
                🔒 {group.name}
              </button>
            );
          })}
            ...
            <div key={prayer.id} style={{ ...css.card, marginBottom:14 }}>
              ...
              {isConfirming ? (
                ...
                <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginTop:4 }}>
                  <button onClick={() => handlePray(prayer)} style={{ background: hasPrayed ? TEAL : "transparent", border:`2px solid ${TEAL}`, borderRadius:22, padding:"8px 18px", cursor:"pointer", display:"flex", alignItems:"center", gap:8 }}>
      ...
      {showForm && (
        <div style={{ position:"absolute", inset:0, background:"rgba(45,43,40,0.55)", zIndex:100, display:"flex", alignItems:"flex-end" }}>
          <div style={{ background:WHITE, borderRadius:"24px 24px 0 0", padding:"28px 24px 44px", width:"100%", boxSizing:"border-box" }}>
            ...
            {activePrivate && (
              <div style={{ background:"rgba(46,125,107,0.08)", border:"1px solid rgba(46,125,107,0.25)", borderRadius:10, padding:"8px 12px", marginBottom:14 }}>
            ...
            <label style={{ display:"flex", alignItems:"center", gap:10, cursor:"pointer", marginBottom:20 }}>
              <input type="checkbox" checked={newPrayer.isAnon} onChange={e => setNewPrayer(p => ({ ...p, isAnon:e.target.checked }))} style={{ width:18, height:18, accentColor:TEAL }}/>
