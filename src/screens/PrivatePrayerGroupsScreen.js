...
function GroupPrayerWall({ group, user, onBack }) {
  ...
  const toggleHeart = async (prayer) => {
    await DB.prayForPrivateGroup(group.id, prayer.id, user.uid);
  };
    ...
    <div style={{ height:"100%", display:"flex", flexDirection:"column", position:"relative" }}>
      <div style={{ padding:"28px 20px 0", flexShrink:0 }}>
        <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:4 }}>
          ...
          <button onClick={() => setShowForm(true)} style={{ background:TEAL, border:"none", borderRadius:"50%", width:40, height:40, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", boxShadow:"0 4px 12px rgba(46,125,107,0.4)", flexShrink:0 }}>
        ...
        {/* Group code badge */}
        {isCreator && (
          <div style={{ background:"rgba(46,125,107,0.1)", border:"1px dashed rgba(46,125,107,0.4)", borderRadius:10, padding:"10px 14px", marginTop:12, marginBottom:4, display:"flex", alignItems:"center", gap:10 }}>
            ...
            <div>
              ...
              <p style={{ fontFamily:"Georgia,serif", fontSize:18, fontWeight:700, color:TEAL, margin:0, letterSpacing:"0.18em" }}>{group.code}</p>
            </div>
        ...
        <div style={{ position:"absolute", inset:0, background:"rgba(45,43,40,0.55)", zIndex:100, display:"flex", alignItems:"flex-end" }}>
          <div style={{ background:WHITE, borderRadius:"24px 24px 0 0", padding:"28px 24px 44px", width:"100%", boxSizing:"border-box" }}>
            ...
            <textarea value={text} onChange={e => setText(e.target.value)}
              placeholder="Share what's on your family's heart…" rows={4}
              style={{ ...css.input, resize:"none", lineHeight:1.7, height:100, marginBottom:14 }} />
            <label style={{ display:"flex", alignItems:"center", gap:10, cursor:"pointer", marginBottom:20 }}>
              <input type="checkbox" checked={isAnon} onChange={e => setIsAnon(e.target.checked)} style={{ width:18, height:18, accentColor:TEAL }} />
  ...
  return (
    <div style={{ height:"100%", overflowY:"auto", padding:"0 20px 100px" }}>
      ...
      <p style={{ fontFamily:"Georgia,serif", fontSize:13, color:MIDGREY, lineHeight:1.6, margin:"0 0 20px" }}>
        Create a private prayer group for your family or inner circle and invite others with a unique code.
      </p>

      {/* Action buttons */}
      <div style={{ display:"flex", gap:10, marginBottom:24 }}>
        <button onClick={() => { setShowCreate(true); setShowJoin(false); setError(""); }} style={{ flex:1, background:TEAL, border:"none", borderRadius:12, padding:"12px 0", fontFamily:"Georgia,serif", fontSize:13, fontWeight:700, color:WHITE, cursor:"pointer" }}>
          + Create Group
        </button>
        <button onClick={() => { setShowJoin(true); setShowCreate(false); setError(""); }} style={{ flex:1, background:"rgba(46,125,107,0.1)", border:`1.5px solid rgba(46,125,107,0.35)`, borderRadius:12, padding:"12px 0", fontFamily:"Georgia,serif", fontSize:13, fontWeight:700, color:TEAL, cursor:"pointer" }}>
          Join with Code
        </button>
      </div>
      ...
      {/* Create form */}
      {showCreate && (
        <div style={{ background:"rgba(255,255,255,0.85)", borderRadius:16, padding:"18px 18px 20px", marginBottom:18, border:`1.5px solid rgba(46,125,107,0.25)` }}>
          <p style={{ fontFamily:"Georgia,serif", fontSize:14, fontWeight:700, color:CHARCOAL, margin:"0 0 12px" }}>New Group Name</p>
          <input value={newName} onChange={e => setNewName(e.target.value)}
            placeholder="e.g. Our Family Circle"
      ...
      {created && (
        <div style={{ background:"rgba(46,125,107,0.1)", border:"1px solid rgba(46,125,107,0.35)", borderRadius:14, padding:"14px 18px", marginBottom:18 }}>
          <p style={{ fontFamily:"Georgia,serif", fontSize:13, fontWeight:600, color:CHARCOAL, margin:"0 0 4px" }}>"{created.name}" created!</p>
          <p style={{ fontFamily:"Georgia,serif", fontSize:12, color:MIDGREY, margin:"0 0 8px" }}>Share this code to invite family members:</p>
          <p style={{ fontFamily:"Georgia,serif", fontSize:24, fontWeight:700, color:TEAL, letterSpacing:"0.2em", margin:"0 0 8px" }}>{created.code}</p>
      ...
      {groups.map(group => {
        ...
        return (
          <div key={group.id} onClick={() => setActiveGroup(group)}
            style={{ background:"rgba(255,255,255,0.7)", borderRadius:14, padding:"16px 18px", marginBottom:10, border:"1.5px solid rgba(46,125,107,0.18)", cursor:"pointer", display:"flex", alignItems:"center", gap:14 }}>
            <div style={{ width:44, height:44, borderRadius:12, background:"rgba(46,125,107,0.12)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:22, flexShrink:0 }}>🔒</div>
            <div style={{ flex:1 }}>
