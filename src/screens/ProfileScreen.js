...
export function ProfileScreen({ user, onLogout, setTab }) {
  ...
  useEffect(() => {
    if (!user) return;
    Promise.all([DB.getSavedDevotions(user.uid), DB.getFamilySessions(user.uid), DB.getPrayedCount(user.uid), DB.getPrivatePrayedCount(user.uid)])
      .then(([devs, sessions, prayed, privatePrayed]) => setStats({ devotions:devs.length, sessions:sessions.length, prayed: prayed + privatePrayed }))
      .catch(() => {});
  ...
  return (
    <div style={{ padding:"0 20px 100px", overflowY:"auto", height:"100%" }}>
      <div style={{ textAlign:"center", padding:"32px 0 24px" }}>
        <div style={{ width:80, height:80, borderRadius:"50%", margin:"0 auto 14px", overflow:"hidden", background:"linear-gradient(135deg,#2E7D6B,#3A9E88)", display:"flex", alignItems:"center", justifyContent:"center", boxShadow:"0 4px 16px rgba(46,125,107,0.4)" }}>
          {user?.photoURL
        ...
        <span style={{ display:"inline-block", background:"rgba(46,125,107,0.12)", color:TEAL, fontFamily:"Georgia,serif", fontSize:11, fontWeight:600, letterSpacing:"0.1em", textTransform:"uppercase", padding:"3px 12px", borderRadius:20 }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:10, marginBottom:18 }}>
        {[
          ["🙏", stats.prayed,    "Prayers Prayed",         () => setTab(3)],
          ["📖", stats.devotions, "Family Devotions",       () => setSubScreen("devotions")],
          ["🌳", stats.sessions,  "Roots Sessions",         () => setTab(2)],
        ].map(([icon, n, label, onPress]) => (
          <div key={label} onClick={onPress} style={{ background:"rgba(255,255,255,0.7)", borderRadius:14, padding:"14px 10px", textAlign:"center", border:"1px solid rgba(46,125,107,0.15)", cursor:"pointer" }}>
            <p style={{ fontSize:18, margin:"0 0 4px" }}>{icon}</p>
            <p style={{ fontFamily:"Georgia,serif", fontSize:22, fontWeight:700, color:TEAL, margin:"0 0 3px" }}>{n}</p>
      ...
      <div style={{ background:"rgba(232,248,232,0.8)", borderRadius:14, padding:"14px 18px", marginBottom:18, border:"1px solid rgba(130,190,130,0.4)" }}>
        ...
        <p style={{ fontFamily:"Georgia,serif", fontSize:12, color:"#4A8A4A", margin:0, lineHeight:1.6 }}>Your prayers, devotions, and Family Roots sessions are saved securely in real time.</p>
      </div>

      {/* Subscription */}
      {subStatus && (
        <div style={{ background:"rgba(255,255,255,0.7)", borderRadius:14, padding:"16px 18px", marginBottom:10, border:"1px solid rgba(46,125,107,0.2)" }}>
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
            ...
            {subStatus !== "promo" && (
              <button onClick={openPortal} disabled={portalLoading} style={{ background:"rgba(46,125,107,0.1)", border:"1px solid rgba(46,125,107,0.3)", borderRadius:10, padding:"8px 14px", fontFamily:"Georgia,serif", fontSize:12, fontWeight:600, color:TEAL, cursor:"pointer", opacity: portalLoading ? 0.6 : 1 }}>
      ...
      {rows.map(({ icon, title, sub, onPress, badge }) => (
        <div
          ...
          style={{ background:"rgba(255,255,255,0.7)", borderRadius:14, padding:"16px 18px", marginBottom:10, border:"1px solid rgba(46,125,107,0.12)", display:"flex", alignItems:"center", gap:14, cursor: onPress ? "pointer" : "default", opacity: onPress ? 1 : 0.5 }}
        >
          ...
          {badge && <span style={{ background:"rgba(46,125,107,0.15)", color:TEAL, fontFamily:"Georgia,serif", fontSize:10, fontWeight:700, padding:"2px 8px", borderRadius:10 }}>{badge}</span>}
          {onPress && <span style={{ color:LTGREY, fontSize:16 }}>›</span>}
        </div>
      ))}

      <button onClick={() => {
        const subject = encodeURIComponent("You should check out Perfectly Flawed Families");
        const body = encodeURIComponent(
          "Hey,\n\nI've been using the Perfectly Flawed Families app and thought you might find it meaningful too.\n\n" +
          "It's a faith-based family tool with devotions tailored for families, a Family Roots advisor, and a prayer wall — all grounded in Scripture.\n\n" +
          "Check it out:\n" +
          "App: https://perfectly-flawed-families.web.app\n" +
          "Website: https://perfectlyflawed.com\n\n" +
          "Hope it encourages your family!"
        );
        window.location.href = `mailto:?subject=${subject}&body=${body}`;
      }} style={{ width:"100%", marginTop:10, background:"rgba(46,125,107,0.08)", border:"1.5px solid rgba(46,125,107,0.3)", borderRadius:13, padding:14, fontFamily:"Georgia,serif", fontSize:14, fontWeight:600, color:TEAL, cursor:"pointer" }}>
        Invite a Friend
      </button>
