...
export const DB = {
  ...
  async getPrivatePrayedCount(uid) {
    const groupsSnap = await getDocs(query(collection(db, "privateGroups"), where("members", "array-contains", uid)));
    if (groupsSnap.empty) return 0;
    const counts = await Promise.all(
      groupsSnap.docs.map(g =>
        getDocs(query(collection(db, "privateGroups", g.id, "prayers"), where("uid", "==", uid)))
          .then(s => s.docs.filter(d => d.data().active !== false).length)
      )
    );
  ...
  async saveFamilySession(uid, session) {
    return addDoc(collection(db, "users", uid, "familySessions"), {
      ...session,
      createdAt: serverTimestamp(),
    });
  },
  async getFamilySessions(uid) {
    const q    = query(collection(db, "users", uid, "familySessions"), orderBy("createdAt", "desc"));
    const snap = await getDocs(q);
    return snap.docs.map(d => ({ id: d.id, ...d.data() }));
  },
